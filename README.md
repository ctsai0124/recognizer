...
- **單一整份月報** → 直接辨識、編碼、生成目次。
- **多個個別報表** → 一次拖入，工具逐檔辨識其主要表、**依目次順序自動排序合併**（同類如對帳單依上傳順序），再接編碼＋目次。跨表順序自動判定，上傳先後不影響。
- 端點 `/api/recognize`、`/api/build` 皆接受多個 `files`；多檔時回傳 `merge_report` 列出各檔判為哪張表。
...
- 上傳戳章圖（PNG/JPG），工具**自動去白底**（背景轉透明、只留印文），蓋在每張「對帳單」頁**右下角**（常見值：寬約 3.5 公分、距邊約 1 公分）。
- 只蓋對帳單，其他表不蓋。與底部置中的頁碼不衝突。
- 產出前可按「預覽蓋章」看去背與位置效果；不滿意可換去背 PNG 再預覽。
- 端點：`/api/stamp_preview`（回 PNG 預覽）；`/api/build` 新增 `stamp_image` 檔即會蓋章。
